/* star1d1r_abft_v2_9_300.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v2_9_300 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N }
 *	 		C2 : {[tt, ti, p]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and -2 <= p <= 2 }
 *	 		I : {[tt, ti]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N }
 *	 		WKernel : {[i]: -2 <= i <= 2 }
 *	 		WCombos : {[i]: -2 <= i <= 2 }
 *	 		WAll : {[w]: 0 < w <= 9 }
 *	 		C2_NR : {[tt, ti, p]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and -2 <= p < 0 }
 *	 		C2_NR2 : {[tt, ti, p]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and 0 < p <= 2 }
 *	 		C2_NR3 : {[tt, ti, p]: p = 0 and tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N }
 *	 		I_NR : {[tt, ti]: tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 300ti <= i <= 317 + 300ti } : Y[9tt,i]);
 *	 		
 *	 		C2[tt,ti,p] = case  {
 *	 			(WCombos[p] * C2_NR);
 *	 			(WCombos[p] * C2_NR2);
 ...
 *	 		I[tt,ti] = ((I_NR - C1) / C1);
 *	 		
 *	 		C2_NR[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[9tt-w,300ti+p+w]));
 *	 		
 *	 		C2_NR2[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[9tt-w,300ti+p-w+317]));
 *	 		
 *	 		C2_NR3[tt,ti,p] = reduce(+, (tt,ti,p,w,i->tt,ti,p), {: w = 9 and 9 + 300ti <= i <= 308 + 300ti } : (WAll[w] * Y[9tt-w,i]));
 *	 		
...
void inject_star1d1r_abft_v2_9_300(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v2_9_300(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v2_9_300(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-27) + T >= ((double)0) && ((double)-619) + N >= ((double)0)) ? (((((((double)2691) + T) + (((double)1) + T) * N) - ((double)1330) * floord(T, ((double)9))) + (((double)-1330) + ((double)665) * floord(T, ((double)9))) * floord(((double)281) + N, ((double)300)))) : ((((double)-27) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)618) - N >= ((double)0)) || (((double)-10) + T >= ((double)0) && ((double)26) - T >= ((double)0) && ((double)-20) + N >= ((double)0))) ? (((((double)31) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	for (int tt=0; tt<((-27 + T >= 0 && -619 + N >= 0) ? ((-2 + floord(T, 9))) : 0); tt++) {
		...
		for (int ti=0; ti<((-27 + T >= 0 && -619 + N >= 0) ? ((-2 + floord(281 + N, 300))) : 0); ti++) {
			C2[tt][ti] = (float*)(calloc(((-27 + T >= 0 && -619 + N >= 0) ? (5) : 0),sizeof(float)));
		}
	}

	#define SC1(tt,ti,i) C1(tt,ti) += Y((9*(tt)),((i)))
	#define SC2_NR(tt,ti,p,w) C2_NR(tt,ti,p) += (WAll(((w)))) * (Y((9*(tt) - (w)),(300*(ti) + (p) + (w))))
	#define SC2_NR2(tt,ti,p,w) C2_NR2(tt,ti,p) += (WAll(((w)))) * (Y((9*(tt) - (w)),(317 + 300*(ti) + (p) - (w))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==9*(tt_INJ-1)+1 && i==300*ti_INJ+159) inject_star1d1r_abft_v2_9_300(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SWKernel[i] : T >= 10 and N >= 20 and -2 <= i <= 2; SC1[tt, ti, i] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and 300ti <= i <= 317 + 300ti; SI[tt, ti] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N; SC2_cb2[tt, ti, p = 0] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SC2_cb1[tt, ti, p] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and 0 < p <= 2; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SC2_cb0[tt, ti, p] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and -2 <= p < 0; SI_NR[tt, ti, p] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and -2 <= p <= 2; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1; SWCombos[i, p] : T >= 10 and N >= 20 and -2 <= i <= 2 and p >= -2 + i and -1 <= p <= 1 and p <= 2 + i; SC2_NR2[tt, ti, p, w] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and 0 < p <= 2 and 0 < w <= 9; SC2_NR3[tt, ti, p = 0, w = 9, i] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and 9 + 300ti <= i <= 308 + 300ti; SWAll[w] : T >= 10 and N >= 20 and 0 < w <= 9; SC2_NR[tt, ti, p, w] : tt >= 2 and 9tt <= -9 + T and ti > 0 and 300ti <= -319 + N and -2 <= p < 0 and 0 < w <= 9 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ SWKernel[i] }"
	...
	for (int c0 = 2; c0 <= T; c0 += 1) {
	  ...
	  if (T >= c0 + 18 && c0 % 9 == 0) {
	    for (int c2 = 1; c2 < (N - 19) / 300; c2 += 1) {
	      for (int c5 = 300 * c2 + 9; c5 <= 300 * c2 + 308; c5 += 1) {
	  ...
	  if (c0 >= 18 && T >= c0 + 9 && c0 % 9 == 0) {
	    for (int c2 = 1; c2 < (N - 19) / 300; c2 += 1) {
	      for (int c3 = 300 * c2; c3 <= 300 * c2 + 317; c3 += 1) {
	        SC1(c0 / 9, c2, c3);
	      }
	    }
	    for (int c2 = 1; c2 < (N - 19) / 300; c2 += 1) {
