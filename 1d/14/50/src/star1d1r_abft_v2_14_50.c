/* star1d1r_abft_v2_14_50.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v2_14_50 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N }
 *	 		C2 : {[tt, ti, p]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and -2 <= p <= 2 }
 *	 		I : {[tt, ti]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N }
 *	 		WKernel : {[i]: -2 <= i <= 2 }
 *	 		WCombos : {[i]: -2 <= i <= 2 }
 *	 		WAll : {[w]: 0 < w <= 14 }
 *	 		C2_NR : {[tt, ti, p]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and -2 <= p < 0 }
 *	 		C2_NR2 : {[tt, ti, p]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and 0 < p <= 2 }
 *	 		C2_NR3 : {[tt, ti, p]: p = 0 and tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N }
 *	 		I_NR : {[tt, ti]: tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 50ti <= i <= 77 + 50ti } : Y[14tt,i]);
 *	 		
 *	 		C2[tt,ti,p] = case  {
 *	 			(WCombos[p] * C2_NR);
 *	 			(WCombos[p] * C2_NR2);
 ...
 *	 		I[tt,ti] = ((I_NR - C1) / C1);
 *	 		
 *	 		C2_NR[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[14tt-w,50ti+p+w]));
 *	 		
 *	 		C2_NR2[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[14tt-w,50ti+p-w+77]));
 *	 		
 *	 		C2_NR3[tt,ti,p] = reduce(+, (tt,ti,p,w,i->tt,ti,p), {: w = 14 and 14 + 50ti <= i <= 63 + 50ti } : (WAll[w] * Y[14tt-w,i]));
 *	 		
...
void inject_star1d1r_abft_v2_14_50(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v2_14_50(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v2_14_50(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-42) + T >= ((double)0) && ((double)-129) + N >= ((double)0)) ? (((((((double)816) + T) + (((double)1) + T) * N) - ((double)390) * floord(T, ((double)14))) + (((double)-390) + ((double)195) * floord(T, ((double)14))) * floord(((double)21) + N, ((double)50)))) : ((((double)-42) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)128) - N >= ((double)0)) || (((double)-10) + T >= ((double)0) && ((double)41) - T >= ((double)0) && ((double)-20) + N >= ((double)0))) ? (((((double)36) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	for (int tt=0; tt<((-42 + T >= 0 && -129 + N >= 0) ? ((-2 + floord(T, 14))) : 0); tt++) {
		...
		for (int ti=0; ti<((-42 + T >= 0 && -129 + N >= 0) ? ((-2 + floord(21 + N, 50))) : 0); ti++) {
			C2[tt][ti] = (float*)(calloc(((-42 + T >= 0 && -129 + N >= 0) ? (5) : 0),sizeof(float)));
		}
	}

	#define SC1(tt,ti,i) C1(tt,ti) += Y((14*(tt)),((i)))
	#define SC2_NR(tt,ti,p,w) C2_NR(tt,ti,p) += (WAll(((w)))) * (Y((14*(tt) - (w)),(50*(ti) + (p) + (w))))
	#define SC2_NR2(tt,ti,p,w) C2_NR2(tt,ti,p) += (WAll(((w)))) * (Y((14*(tt) - (w)),(77 + 50*(ti) + (p) - (w))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==14*(tt_INJ-1)+1 && i==50*ti_INJ+39) inject_star1d1r_abft_v2_14_50(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SWKernel[i] : T >= 10 and N >= 20 and -2 <= i <= 2; SC1[tt, ti, i] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and 50ti <= i <= 77 + 50ti; SI[tt, ti] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N; SC2_cb2[tt, ti, p = 0] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SC2_cb1[tt, ti, p] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and 0 < p <= 2; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SC2_cb0[tt, ti, p] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and -2 <= p < 0; SI_NR[tt, ti, p] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and -2 <= p <= 2; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1; SWCombos[i, p] : T >= 10 and N >= 20 and -2 <= i <= 2 and p >= -2 + i and -1 <= p <= 1 and p <= 2 + i; SC2_NR2[tt, ti, p, w] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and 0 < p <= 2 and 0 < w <= 14; SC2_NR3[tt, ti, p = 0, w = 14, i] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and 14 + 50ti <= i <= 63 + 50ti; SWAll[w] : T >= 10 and N >= 20 and 0 < w <= 14; SC2_NR[tt, ti, p, w] : tt >= 2 and 14tt <= -14 + T and ti > 0 and 50ti <= -79 + N and -2 <= p < 0 and 0 < w <= 14 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ SWKernel[i] }"
	...
	for (int c0 = 2; c0 <= T; c0 += 1) {
	  ...
	  if (c0 >= 28 && T >= c0 + 14 && c0 % 14 == 0) {
	    for (int c2 = 1; c2 < (N + 21) / 50 - 1; c2 += 1) {
	      for (int c3 = 50 * c2; c3 <= 50 * c2 + 77; c3 += 1) {
	        SC1(c0 / 14, c2, c3);
	      }
	    }
	  }
	  if (T >= c0 + 28 && c0 % 14 == 0) {
	    for (int c2 = 1; c2 < (N + 21) / 50 - 1; c2 += 1) {
	      for (int c5 = 50 * c2 + 14; c5 <= 50 * c2 + 63; c5 += 1) {
