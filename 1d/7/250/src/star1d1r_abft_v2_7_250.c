/* star1d1r_abft_v2_7_250.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v2_7_250 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N }
 *	 		C2 : {[tt, ti, p]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and -2 <= p <= 2 }
 *	 		I : {[tt, ti]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N }
 *	 		WKernel : {[i]: -2 <= i <= 2 }
 *	 		WCombos : {[i]: -2 <= i <= 2 }
 *	 		WAll : {[w]: 0 < w <= 7 }
 *	 		C2_NR : {[tt, ti, p]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and -2 <= p < 0 }
 *	 		C2_NR2 : {[tt, ti, p]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and 0 < p <= 2 }
 *	 		C2_NR3 : {[tt, ti, p]: p = 0 and tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N }
 *	 		I_NR : {[tt, ti]: tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 250ti <= i <= 263 + 250ti } : Y[7tt,i]);
 *	 		
 *	 		C2[tt,ti,p] = case  {
 *	 			(WCombos[p] * C2_NR);
 *	 			(WCombos[p] * C2_NR2);
 ...
 *	 		I[tt,ti] = ((I_NR - C1) / C1);
 *	 		
 *	 		C2_NR[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[7tt-w,250ti+p+w]));
 *	 		
 *	 		C2_NR2[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[7tt-w,250ti+p-w+263]));
 *	 		
 *	 		C2_NR3[tt,ti,p] = reduce(+, (tt,ti,p,w,i->tt,ti,p), {: w = 7 and 7 + 250ti <= i <= 256 + 250ti } : (WAll[w] * Y[7tt-w,i]));
 *	 		
...
void inject_star1d1r_abft_v2_7_250(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v2_7_250(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v2_7_250(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-21) + T >= ((double)0) && ((double)-515) + N >= ((double)0)) ? (((((((double)2241) + T) + (((double)1) + T) * N) - ((double)1106) * floord(T, ((double)7))) + (((double)-1106) + ((double)553) * floord(T, ((double)7))) * floord(((double)235) + N, ((double)250)))) : ((((double)-21) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)514) - N >= ((double)0)) || (((double)-10) + T >= ((double)0) && ((double)20) - T >= ((double)0) && ((double)-20) + N >= ((double)0))) ? (((((double)29) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	for (int tt=0; tt<((-21 + T >= 0 && -515 + N >= 0) ? ((-2 + floord(T, 7))) : 0); tt++) {
		...
		for (int ti=0; ti<((-21 + T >= 0 && -515 + N >= 0) ? ((-2 + floord(235 + N, 250))) : 0); ti++) {
			C2[tt][ti] = (float*)(calloc(((-21 + T >= 0 && -515 + N >= 0) ? (5) : 0),sizeof(float)));
		}
	}

	#define SC1(tt,ti,i) C1(tt,ti) += Y((7*(tt)),((i)))
	#define SC2_NR(tt,ti,p,w) C2_NR(tt,ti,p) += (WAll(((w)))) * (Y((7*(tt) - (w)),(250*(ti) + (p) + (w))))
	#define SC2_NR2(tt,ti,p,w) C2_NR2(tt,ti,p) += (WAll(((w)))) * (Y((7*(tt) - (w)),(263 + 250*(ti) + (p) - (w))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==7*(tt_INJ-1)+1 && i==250*ti_INJ+132) inject_star1d1r_abft_v2_7_250(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SWKernel[i] : T >= 10 and N >= 20 and -2 <= i <= 2; SC1[tt, ti, i] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and 250ti <= i <= 263 + 250ti; SI[tt, ti] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N; SC2_cb2[tt, ti, p = 0] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SC2_cb1[tt, ti, p] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and 0 < p <= 2; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SC2_cb0[tt, ti, p] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and -2 <= p < 0; SI_NR[tt, ti, p] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and -2 <= p <= 2; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1; SWCombos[i, p] : T >= 10 and N >= 20 and -2 <= i <= 2 and p >= -2 + i and -1 <= p <= 1 and p <= 2 + i; SC2_NR2[tt, ti, p, w] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and 0 < p <= 2 and 0 < w <= 7; SC2_NR3[tt, ti, p = 0, w = 7, i] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and 7 + 250ti <= i <= 256 + 250ti; SWAll[w] : T >= 10 and N >= 20 and 0 < w <= 7; SC2_NR[tt, ti, p, w] : tt >= 2 and 7tt <= -7 + T and ti > 0 and 250ti <= -265 + N and -2 <= p < 0 and 0 < w <= 7 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ SWKernel[i] }"
	...
	for (int c0 = 2; c0 <= T; c0 += 1) {
	  ...
	  if (T >= c0 + 14 && c0 % 7 == 0) {
	    for (int c2 = 1; c2 < (N - 15) / 250; c2 += 1) {
	      for (int c5 = 250 * c2 + 7; c5 <= 250 * c2 + 256; c5 += 1) {
	  ...
	  if (c0 >= 14 && T >= c0 + 7 && c0 % 7 == 0) {
	    for (int c2 = 1; c2 < (N - 15) / 250; c2 += 1) {
	      for (int c3 = 250 * c2; c3 <= 250 * c2 + 263; c3 += 1) {
	        SC1(c0 / 7, c2, c3);
	      }
	    }
	    for (int c2 = 1; c2 < (N - 15) / 250; c2 += 1) {
