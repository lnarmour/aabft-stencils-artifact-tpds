/* star1d1r_abft_v2_10_450.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v2_10_450 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N }
 *	 		C2 : {[tt, ti, p]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and -2 <= p <= 2 }
 *	 		I : {[tt, ti]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N }
 *	 		WKernel : {[i]: -2 <= i <= 2 }
 *	 		WCombos : {[i]: -2 <= i <= 2 }
 *	 		WAll : {[w]: 0 < w <= 10 }
 *	 		C2_NR : {[tt, ti, p]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and -2 <= p < 0 }
 *	 		C2_NR2 : {[tt, ti, p]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and 0 < p <= 2 }
 *	 		C2_NR3 : {[tt, ti, p]: p = 0 and tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N }
 *	 		I_NR : {[tt, ti]: tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 450ti <= i <= 469 + 450ti } : Y[10tt,i]);
 *	 		
 *	 		C2[tt,ti,p] = case  {
 *	 			(WCombos[p] * C2_NR);
 *	 			(WCombos[p] * C2_NR2);
 ...
 *	 		I[tt,ti] = ((I_NR - C1) / C1);
 *	 		
 *	 		C2_NR[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[10tt-w,450ti+p+w]));
 *	 		
 *	 		C2_NR2[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[10tt-w,450ti+p-w+469]));
 *	 		
 *	 		C2_NR3[tt,ti,p] = reduce(+, (tt,ti,p,w,i->tt,ti,p), {: w = 10 and 10 + 450ti <= i <= 459 + 450ti } : (WAll[w] * Y[10tt-w,i]));
 *	 		
...
void inject_star1d1r_abft_v2_10_450(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v2_10_450(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v2_10_450(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-30) + T >= ((double)0) && ((double)-921) + N >= ((double)0)) ? (((((((double)3916) + T) + (((double)1) + T) * N) - ((double)1942) * floord(T, ((double)10))) + (((double)-1942) + ((double)971) * floord(T, ((double)10))) * floord(((double)429) + N, ((double)450)))) : ((((double)-30) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)920) - N >= ((double)0)) || (((double)-10) + T >= ((double)0) && ((double)29) - T >= ((double)0) && ((double)-20) + N >= ((double)0))) ? (((((double)32) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	for (int tt=0; tt<((-30 + T >= 0 && -921 + N >= 0) ? ((-2 + floord(T, 10))) : 0); tt++) {
		...
		for (int ti=0; ti<((-30 + T >= 0 && -921 + N >= 0) ? ((-2 + floord(429 + N, 450))) : 0); ti++) {
			C2[tt][ti] = (float*)(calloc(((-30 + T >= 0 && -921 + N >= 0) ? (5) : 0),sizeof(float)));
		}
	}

	#define SC1(tt,ti,i) C1(tt,ti) += Y((10*(tt)),((i)))
	#define SC2_NR(tt,ti,p,w) C2_NR(tt,ti,p) += (WAll(((w)))) * (Y((10*(tt) - (w)),(450*(ti) + (p) + (w))))
	#define SC2_NR2(tt,ti,p,w) C2_NR2(tt,ti,p) += (WAll(((w)))) * (Y((10*(tt) - (w)),(469 + 450*(ti) + (p) - (w))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==10*(tt_INJ-1)+1 && i==450*ti_INJ+235) inject_star1d1r_abft_v2_10_450(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SWKernel[i] : T >= 10 and N >= 20 and -2 <= i <= 2; SC1[tt, ti, i] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and 450ti <= i <= 469 + 450ti; SI[tt, ti] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N; SC2_cb2[tt, ti, p = 0] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SC2_cb1[tt, ti, p] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and 0 < p <= 2; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SC2_cb0[tt, ti, p] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and -2 <= p < 0; SI_NR[tt, ti, p] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and -2 <= p <= 2; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1; SWCombos[i, p] : T >= 10 and N >= 20 and -2 <= i <= 2 and p >= -2 + i and -1 <= p <= 1 and p <= 2 + i; SC2_NR2[tt, ti, p, w] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and 0 < p <= 2 and 0 < w <= 10; SC2_NR3[tt, ti, p = 0, w = 10, i] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and 10 + 450ti <= i <= 459 + 450ti; SWAll[w] : T >= 10 and N >= 20 and 0 < w <= 10; SC2_NR[tt, ti, p, w] : tt >= 2 and 10tt <= -10 + T and ti > 0 and 450ti <= -471 + N and -2 <= p < 0 and 0 < w <= 10 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ SWKernel[i] }"
	...
	for (int c0 = 2; c0 <= T; c0 += 1) {
	  ...
	  if (T >= c0 + 20 && c0 % 10 == 0) {
	    for (int c2 = 1; c2 < floord(N - 21, 450); c2 += 1) {
	      for (int c5 = 450 * c2 + 10; c5 <= 450 * c2 + 459; c5 += 1) {
	  ...
	  if (c0 >= 20 && T >= c0 + 10 && c0 % 10 == 0) {
	    for (int c2 = 1; c2 < floord(N - 21, 450); c2 += 1) {
	      for (int c3 = 450 * c2; c3 <= 450 * c2 + 469; c3 += 1) {
	        SC1(c0 / 10, c2, c3);
	      }
	    }
	  }
