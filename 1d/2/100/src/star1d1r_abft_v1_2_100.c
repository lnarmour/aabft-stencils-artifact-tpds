/* star1d1r_abft_v1_2_100.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v1_2_100 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N }
 *	 		C2 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N }
 *	 		I : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N }
 *	 		patch : {[w, i]: 0 <= w <= 2 and -1 - w <= i <= 100 + w }
 *	 		patch_NR : {[w, i]: 0 < w <= 2 and -1 - w <= i <= 100 + w }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 100ti <= i <= 99 + 100ti } : Y[2tt,i]);
 *	 		
 *	 		C2[tt,ti] = reduce(+, (tt,ti,p->tt,ti), (patch[2,p] * Y[2tt-2,100ti+p]));
 *	 		
 *	 		I[tt,ti] = ((C2 - C1) / C1);
 *	 		
...
void inject_star1d1r_abft_v1_2_100(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v1_2_100(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v1_2_100(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-10) + T >= ((double)0) && ((double)-202) + N >= ((double)0)) ? ((((((((double)1762) + T) + (((double)1) + T) * N) - ((double)414) * floord(T, ((double)2))) + ((((double)96) + ((double)2) * N) + (((double)-48) - N) * floord(T, ((double)2))) * floord(((double)97) + N, ((double)100)) + (((double)-100) + ((double)50) * floord(T, ((double)2))) * floord(((double)97) + N, ((double)100))*floord(((double)97) + N, ((double)100))) + ((((double)-510) - ((double)2) * N) + (((double)255) + N) * floord(T, ((double)2))) * floord(((double)99) + N, ((double)100)) + (((double)100) - ((double)50) * floord(T, ((double)2))) * floord(((double)99) + N, ((double)100))*floord(((double)99) + N, ((double)100)))) : (((double)-201) + N == ((double)0) && ((double)-10) + T >= ((double)0)) ? (((((double)723) + ((double)202) * T) + ((double)206) * floord(T, ((double)2)))) : (((double)-10) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)200) - N >= ((double)0)) ? (((((double)934) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	#define SC2(tt,ti,p) C2(tt,ti) += (patch((2),((p)))) * (Y((-2 + 2*(tt)),(100*(ti) + (p))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==2*(tt_INJ-1)+1 && i==100*ti_INJ+50) inject_star1d1r_abft_v1_2_100(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SI[tt, ti] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N; SC1[tt, ti, i] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N and 100ti <= i <= 99 + 100ti; SC2[tt, ti, p] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 100ti <= -101 + N and -3 <= p <= 102 and p <= N - 100ti; Spatch_NR[w, i, p] : T >= 10 and N >= 20 and 0 < w <= 2 and p >= -1 and -99 - w + i <= p <= 1 and p <= w + i; Spatch[w, i] : T >= 10 and N >= 20 and 0 <= w <= 2 and -1 - w <= i <= 100 + w; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ Spatch[w, i]; Spatch_NR[w, i, p] }"
	  ...
	  for (int c1 = 2 * c0; c1 <= min(T, 2 * c0 + 1); c1 += 1) {
	    ...
	    if (T >= 2 * c0 + 4 && c1 == 2 * c0) {
	      for (int c3 = 1; c3 < (N - 1) / 100; c3 += 1) {
	        for (int c4 = -3; c4 <= min(102, N - 100 * c3); c4 += 1) {
	          SC2(c0 + 1, c3, c4);
	        }
	      }
	    }
