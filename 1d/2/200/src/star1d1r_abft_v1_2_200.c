/* star1d1r_abft_v1_2_200.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v1_2_200 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N }
 *	 		C2 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N }
 *	 		I : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N }
 *	 		patch : {[w, i]: 0 <= w <= 2 and -1 - w <= i <= 200 + w }
 *	 		patch_NR : {[w, i]: 0 < w <= 2 and -1 - w <= i <= 200 + w }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 200ti <= i <= 199 + 200ti } : Y[2tt,i]);
 *	 		
 *	 		C2[tt,ti] = reduce(+, (tt,ti,p->tt,ti), (patch[2,p] * Y[2tt-2,200ti+p]));
 *	 		
 *	 		I[tt,ti] = ((C2 - C1) / C1);
 *	 		
...
void inject_star1d1r_abft_v1_2_200(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v1_2_200(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v1_2_200(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-10) + T >= ((double)0) && ((double)-402) + N >= ((double)0)) ? ((((((((double)3462) + T) + (((double)1) + T) * N) - ((double)814) * floord(T, ((double)2))) + ((((double)196) + ((double)2) * N) + (((double)-98) - N) * floord(T, ((double)2))) * floord(((double)197) + N, ((double)200)) + (((double)-200) + ((double)100) * floord(T, ((double)2))) * floord(((double)197) + N, ((double)200))*floord(((double)197) + N, ((double)200))) + ((((double)-1010) - ((double)2) * N) + (((double)505) + N) * floord(T, ((double)2))) * floord(((double)199) + N, ((double)200)) + (((double)200) - ((double)100) * floord(T, ((double)2))) * floord(((double)199) + N, ((double)200))*floord(((double)199) + N, ((double)200)))) : (((double)-401) + N == ((double)0) && ((double)-10) + T >= ((double)0)) ? (((((double)1423) + ((double)402) * T) + ((double)406) * floord(T, ((double)2)))) : (((double)-10) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)400) - N >= ((double)0)) ? (((((double)1834) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	#define SC2(tt,ti,p) C2(tt,ti) += (patch((2),((p)))) * (Y((-2 + 2*(tt)),(200*(ti) + (p))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==2*(tt_INJ-1)+1 && i==200*ti_INJ+100) inject_star1d1r_abft_v1_2_200(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SI[tt, ti] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N; SC1[tt, ti, i] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N and 200ti <= i <= 199 + 200ti; SC2[tt, ti, p] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 200ti <= -201 + N and -3 <= p <= 202 and p <= N - 200ti; Spatch_NR[w, i, p] : T >= 10 and N >= 20 and 0 < w <= 2 and p >= -1 and -199 - w + i <= p <= 1 and p <= w + i; Spatch[w, i] : T >= 10 and N >= 20 and 0 <= w <= 2 and -1 - w <= i <= 200 + w; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ Spatch[w, i]; Spatch_NR[w, i, p] }"
	  ...
	  for (int c1 = 2 * c0; c1 <= min(T, 2 * c0 + 1); c1 += 1) {
	    ...
	    if (T >= 2 * c0 + 4 && c1 == 2 * c0) {
	      for (int c3 = 1; c3 < (N - 1) / 200; c3 += 1) {
	        for (int c4 = -3; c4 <= min(202, N - 200 * c3); c4 += 1) {
	          SC2(c0 + 1, c3, c4);
	        }
	      }
	    }
