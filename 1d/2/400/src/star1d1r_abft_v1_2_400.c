/* star1d1r_abft_v1_2_400.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v1_2_400 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N }
 *	 		C2 : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N }
 *	 		I : {[tt, ti]: tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N }
 *	 		patch : {[w, i]: 0 <= w <= 2 and -1 - w <= i <= 400 + w }
 *	 		patch_NR : {[w, i]: 0 < w <= 2 and -1 - w <= i <= 400 + w }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 400ti <= i <= 399 + 400ti } : Y[2tt,i]);
 *	 		
 *	 		C2[tt,ti] = reduce(+, (tt,ti,p->tt,ti), (patch[2,p] * Y[2tt-2,400ti+p]));
 *	 		
 *	 		I[tt,ti] = ((C2 - C1) / C1);
 *	 		
...
void inject_star1d1r_abft_v1_2_400(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v1_2_400(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v1_2_400(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-10) + T >= ((double)0) && ((double)-802) + N >= ((double)0)) ? ((((((((double)6862) + T) + (((double)1) + T) * N) - ((double)1614) * floord(T, ((double)2))) + ((((double)396) + ((double)2) * N) + (((double)-198) - N) * floord(T, ((double)2))) * floord(((double)397) + N, ((double)400)) + (((double)-400) + ((double)200) * floord(T, ((double)2))) * floord(((double)397) + N, ((double)400))*floord(((double)397) + N, ((double)400))) + ((((double)-2010) - ((double)2) * N) + (((double)1005) + N) * floord(T, ((double)2))) * floord(((double)399) + N, ((double)400)) + (((double)400) - ((double)200) * floord(T, ((double)2))) * floord(((double)399) + N, ((double)400))*floord(((double)399) + N, ((double)400)))) : (((double)-801) + N == ((double)0) && ((double)-10) + T >= ((double)0)) ? (((((double)2823) + ((double)802) * T) + ((double)806) * floord(T, ((double)2)))) : (((double)-10) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)800) - N >= ((double)0)) ? (((((double)3634) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	#define SC2(tt,ti,p) C2(tt,ti) += (patch((2),((p)))) * (Y((-2 + 2*(tt)),(400*(ti) + (p))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==2*(tt_INJ-1)+1 && i==400*ti_INJ+200) inject_star1d1r_abft_v1_2_400(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SI[tt, ti] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N; SC1[tt, ti, i] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N and 400ti <= i <= 399 + 400ti; SC2[tt, ti, p] : T >= 10 and tt >= 2 and 2tt <= -2 + T and ti > 0 and 400ti <= -401 + N and -3 <= p <= 402 and p <= N - 400ti; Spatch_NR[w, i, p] : T >= 10 and N >= 20 and 0 < w <= 2 and p >= -1 and -399 - w + i <= p <= 1 and p <= w + i; Spatch[w, i] : T >= 10 and N >= 20 and 0 <= w <= 2 and -1 - w <= i <= 400 + w; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ Spatch[w, i]; Spatch_NR[w, i, p] }"
	  ...
	  for (int c1 = 2 * c0; c1 <= min(T, 2 * c0 + 1); c1 += 1) {
	    ...
	    if (T >= 2 * c0 + 4 && c1 == 2 * c0) {
	      for (int c3 = 1; c3 < (N - 1) / 400; c3 += 1) {
	        for (int c4 = -3; c4 <= min(402, N - 400 * c3); c4 += 1) {
	          SC2(c0 + 1, c3, c4);
	        }
	      }
	    }
