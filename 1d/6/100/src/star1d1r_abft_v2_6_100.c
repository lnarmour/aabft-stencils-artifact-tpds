/* star1d1r_abft_v2_6_100.c
 * 
 * Code generation of the following Alpha system:
 *	 affine star1d1r_abft_v2_6_100 [T, N] -> {  : T >= 10 and N >= 20 }
 *	 	inputs
 *	 		X : {[i]: 0 <= i <= N }
 *	 	outputs
 ...
 *	 	locals
 *	 		W : {[i]: -1 <= i <= 1 }
 *	 		C1 : {[tt, ti]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N }
 *	 		C2 : {[tt, ti, p]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and -2 <= p <= 2 }
 *	 		I : {[tt, ti]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N }
 *	 		WKernel : {[i]: -2 <= i <= 2 }
 *	 		WCombos : {[i]: -2 <= i <= 2 }
 *	 		WAll : {[w]: 0 < w <= 6 }
 *	 		C2_NR : {[tt, ti, p]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and -2 <= p < 0 }
 *	 		C2_NR2 : {[tt, ti, p]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and 0 < p <= 2 }
 *	 		C2_NR3 : {[tt, ti, p]: p = 0 and tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N }
 *	 		I_NR : {[tt, ti]: tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N }
 ...
 *	 		};
 *	 		
 *	 		C1[tt,ti] = reduce(+, (tt,ti,i->tt,ti), {: 100ti <= i <= 111 + 100ti } : Y[6tt,i]);
 *	 		
 *	 		C2[tt,ti,p] = case  {
 *	 			(WCombos[p] * C2_NR);
 *	 			(WCombos[p] * C2_NR2);
 ...
 *	 		I[tt,ti] = ((I_NR - C1) / C1);
 *	 		
 *	 		C2_NR[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[6tt-w,100ti+p+w]));
 *	 		
 *	 		C2_NR2[tt,ti,p] = reduce(+, (tt,ti,p,w->tt,ti,p), (WAll[w] * Y[6tt-w,100ti+p-w+111]));
 *	 		
 *	 		C2_NR3[tt,ti,p] = reduce(+, (tt,ti,p,w,i->tt,ti,p), {: w = 6 and 6 + 100ti <= i <= 105 + 100ti } : (WAll[w] * Y[6tt-w,i]));
 *	 		
...
void inject_star1d1r_abft_v2_6_100(float *val) {
	int *bits;
	bits = (int*)val;
	*bits ^= 1 << BIT;
}
...
void star1d1r_abft_v2_6_100(double T, double N, float *X, float **Y)
#else
struct Result star1d1r_abft_v2_6_100(long T, long N, float *X, float **Y)
#endif
{
	#if defined REPORT_COMPLEXITY_ONLY
	#undef ceild
	...
	double total_card = ((((double)-18) + T >= ((double)0) && ((double)-213) + N >= ((double)0)) ? (((((((double)1016) + T) + (((double)1) + T) * N) - ((double)494) * floord(T, ((double)6))) + (((double)-494) + ((double)247) * floord(T, ((double)6))) * floord(((double)87) + N, ((double)100)))) : ((((double)-18) + T >= ((double)0) && ((double)-20) + N >= ((double)0) && ((double)212) - N >= ((double)0)) || (((double)-10) + T >= ((double)0) && ((double)17) - T >= ((double)0) && ((double)-20) + N >= ((double)0))) ? (((((double)28) + T) + (((double)1) + T) * N)) : ((double)0));
	...
	for (int tt=0; tt<((-18 + T >= 0 && -213 + N >= 0) ? ((-2 + floord(T, 6))) : 0); tt++) {
		...
		for (int ti=0; ti<((-18 + T >= 0 && -213 + N >= 0) ? ((-2 + floord(87 + N, 100))) : 0); ti++) {
			C2[tt][ti] = (float*)(calloc(((-18 + T >= 0 && -213 + N >= 0) ? (5) : 0),sizeof(float)));
		}
	}

	#define SC1(tt,ti,i) C1(tt,ti) += Y((6*(tt)),((i)))
	#define SC2_NR(tt,ti,p,w) C2_NR(tt,ti,p) += (WAll(((w)))) * (Y((6*(tt) - (w)),(100*(ti) + (p) + (w))))
	#define SC2_NR2(tt,ti,p,w) C2_NR2(tt,ti,p) += (WAll(((w)))) * (Y((6*(tt) - (w)),(111 + 100*(ti) + (p) - (w))))
	...
	#ifdef ERROR_INJECTION
	#define SY_cb2(t,i) do { SY_cb2_hook(t,i); if (t==6*(tt_INJ-1)+1 && i==100*ti_INJ+56) inject_star1d1r_abft_v2_6_100(&Y(t,i)); } while(0)
	#else
	...
	/*
	 * domain: "[T, N] -> { SY_cb0[t, i] : T >= 10 and N >= 20 and 0 <= t <= 1 and 0 <= i <= N; SWKernel[i] : T >= 10 and N >= 20 and -2 <= i <= 2; SC1[tt, ti, i] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and 100ti <= i <= 111 + 100ti; SI[tt, ti] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N; SC2_cb2[tt, ti, p = 0] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N; SY_cb2[t, i] : T >= 10 and N >= 20 and 2 <= t <= T and 0 < i < N; SC2_cb1[tt, ti, p] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and 0 < p <= 2; SY_cb1[t, i = N] : T >= 10 and N >= 20 and 2 <= t <= T; SY_cb1[t, i = 0] : T >= 10 and N >= 20 and 2 <= t <= T; SC2_cb0[tt, ti, p] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and -2 <= p < 0; SI_NR[tt, ti, p] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and -2 <= p <= 2; SW[i] : T >= 10 and N >= 20 and -1 <= i <= 1; SWCombos[i, p] : T >= 10 and N >= 20 and -2 <= i <= 2 and p >= -2 + i and -1 <= p <= 1 and p <= 2 + i; SC2_NR2[tt, ti, p, w] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and 0 < p <= 2 and 0 < w <= 6; SC2_NR3[tt, ti, p = 0, w = 6, i] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and 6 + 100ti <= i <= 105 + 100ti; SWAll[w] : T >= 10 and N >= 20 and 0 < w <= 6; SC2_NR[tt, ti, p, w] : tt >= 2 and 6tt <= -6 + T and ti > 0 and 100ti <= -113 + N and -2 <= p < 0 and 0 < w <= 6 }"
	 * child:
	 *   sequence:
	 *   - filter: "{ SW[i] }"
	 *   - filter: "{ SWKernel[i] }"
	...
	for (int c0 = 2; c0 <= T; c0 += 1) {
	  ...
	  if (c0 >= 6 && (c0 % 6) + T >= c0 + 12) {
	    ...
	    if (c0 % 6 == 0) {
	      for (int c2 = 1; c2 < (N - 13) / 100; c2 += 1) {
	        for (int c5 = 100 * c2 + 6; c5 <= 100 * c2 + 105; c5 += 1) {
	  ...
	  if (c0 >= 12 && T >= c0 + 6 && c0 % 6 == 0) {
	    for (int c2 = 1; c2 < (N - 13) / 100; c2 += 1) {
	      for (int c3 = 100 * c2; c3 <= 100 * c2 + 111; c3 += 1) {
	        SC1(c0 / 6, c2, c3);
	      }
	    }
	    for (int c2 = 1; c2 < (N - 13) / 100; c2 += 1) {
